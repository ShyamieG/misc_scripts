# AUTHOR: SSG
# This script accepts the output of plink --genome, a .fam file, and a relatedness threshold
...
OUTPUT <- paste(args[1], "relateds", sep="_")
...
min.related.set <- function(kinship.data, pihat.max) {
  ...
  while (nrow(relateds.data)>=1) {
    ...
    relateds.data <- relateds.data[-c(grep(ID, relateds.data[,"IID1"]),
                                      grep(ID, relateds.data[,"IID2"])),]
    relateds <- c(relateds, ID)
  }
  return(relateds)
}

relateds <- min.related.set(KINSHIP.DAT, pihat.max)
RELATED.FAM <- FAM[match(relateds, FAM[,2]),]
write.table(RELATED.FAM, OUTPUT, quote=F, row.names=F, col.names=F, sep=" ")
